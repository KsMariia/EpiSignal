...
In a 'sample.dataset' folder you can find a test count matrix, which contains two wild type samples of CD8+ T-cells H3K4me2 ChIP-seq in columns downsampled to 500 genes, with the corresponding R script for running EpiSignal algorithm in a 'test_run.R' file. The output is represented in an output folder.
